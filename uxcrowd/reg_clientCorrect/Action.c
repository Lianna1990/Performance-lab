Action()
{
	...
	lr_save_string (lr_eval_string ("{param}"), "param_to_log");
  	lr_log_message ("client%s@test/.com", lr_eval_string ("{param_to_log}")) //исправлена ошибка,был неверно указан /
  	
	FileVarriable = fopen (FileLocation,"a+");
    fprintf (FileVarriable, "client%s@test/.com\n", lr_eval_string ("{param_to_log}")); //исправлена ошибка,был неверно указан /
